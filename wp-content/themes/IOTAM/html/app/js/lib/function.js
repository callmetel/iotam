...
var status = 1;
	...
	//Intro Animation
		...
		function intro(){
			tm.set('.initial-hide',{zIndex:1, display:'inherit', alpha:0});
			tm.to('.initial-hide',1, {alpha:1, ease:Power1.easeInOut, delay:.5});
			...
			tm.set('#main', {className:'+=is-faded-in-Y', delay:1.5});
		...
		$('#menu > li > span').on('click', function(){
			if ($(this).parent().children('ul').hasClass('dropdown') && !$(this).children('ul').hasClass('is-open')){
				$(this).parent().children('ul.dropdown').addClass('is-open');
			}
		});

		$('.dropdown > li.has-children > .dropdown-title').on("click", function() {
			$(this).parent().siblings().find('ul').addClass("hidden");
			$(this).parent().find('ul').stop(true, false, true).removeClass('hidden');
			return false;
			$(this).parent().siblings().find('ul').addClass("hidden");
	...
	// Nav Toggle Animation 
		...
		$('#invisible-menu-btn, #menu-btn-rect, #m-menu-btn, #close-menu-btn, .main-link').click(function(){
	...
	//Hover Events
		...
		$('.menu-item-number:eq(3), .menu-item-title:eq(3)').click(function(){
			...
			$("#dot2").on("click",function() {
				$(".first-square").fadeOut(3);
				$(".second-square").fadeTo('slow/600/slow', 1, function() {
				});
			});
	
			$("#dot1").on("click",function() {
				$(".second-square").css("display", "none");
				$(".first-square").fadeTo('slow/600/slow', 1, function() {
